...
export const UserFollowing = ({
  ...
  const followingUsers = async () => {
    const data = await fetch(
      `https://ig-backend-eop9.onrender.com/user/following/${userId}`,
      {
        method: "GET",
        headers: {
  ...
  return (
    <div>
      <Dialog open={open} onOpenChange={(e) => setOpen(e)}>
        <DialogContent className="font-bold text-2xl bg-black">
          following
          <DialogTitle>
            {following?.map((following, index) => {
              return (
                <div key={index} className="flex" style={{ gap: "20px" }}>
                  ...
                  <div className="flex flex-col" style={{ paddingTop: "15px" }}>
                    <div
                      ...
                      onClick={() => router.push(`/user/post/${following._id}`)}
                    >
                      {following.username}
                    </div>
                    <div
