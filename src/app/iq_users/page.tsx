...
const Page = () => {
  ...
  const igUsers = async () => {
    const data = await fetch("https://ig-backend-eop9.onrender.com/user/post");
  ...
  const userSearching = users.filter((user) => {
    ...
    <Parent>
      <div className="bg-black h-[900px]">
        <Input
          ...
          className="text-white sticky top-0 z-10 bg-black"
          placeholder="search"
        ></Input>
        <div>
          {userSearching?.map((user) => {
            return (
              <Card
                key={user._id}
                className="bg-black border-black [90px] overscroll-contain"
              >
                <CardHeader>
